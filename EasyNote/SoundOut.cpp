...
void CSoundOut::Stop()
{
	...
	if(m_bPlaying)
	{
		...
		if (mmReturn) waveOutErrorMsg(mmReturn, _T("in stop()"));
		TRACE("waveOutReset()\n");
	}
}
...
int CSoundOut::AddOutputBufferToQueue(CBuffer *buffer)
{
	...
	if (mmReturn) waveOutErrorMsg(mmReturn, _T("in AddOutputBufferToQueue()"));
...
void CSoundOut::waveOutErrorMsg(MMRESULT result, LPCTSTR addstr)
{
	...
	waveOutGetErrorText(result, (LPWSTR)errorbuffer,100);
	ErrorMsg(_T("WAVEOUT:%x:%s %s"), result, errorbuffer, addstr);
}
