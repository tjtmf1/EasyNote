...
void CPipe::StartRecordingToFile(int num)
{
	CTime ctime = CTime::GetCurrentTime();
	CString str;
	str.Format(_T("%02d%02d%02d.wav"), ctime.GetMonth(), ctime.GetDay(), num);
	m_pFile = new CSoundFile(str, m_SoundIn.GetFormat());
...
void CPipe::StartPlayingFromFile(int num)
{
	CTime ctime = CTime::GetCurrentTime();
	CString str;
	str.Format(_T("%02d%02d%02d.wav"), ctime.GetMonth(), ctime.GetDay(), num);
	m_pFile = new CSoundFile(str);

	if(m_pFile && m_pFile->IsOK())
		m_SoundOut.Start(m_pFile->GetFormat());
	else
		ErrorMsg(_T("Unable to open file"));
}
