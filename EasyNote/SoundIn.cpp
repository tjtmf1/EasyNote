...
void CSoundIn::Stop()
{
	...
	else
	{
		...
		//m_QueuedBuffers = 0;
		if (m_QueuedBuffers != 0) ErrorMsg(_T("Still %d buffers in waveIn queue!"), m_QueuedBuffers);
	}
}
...
void CSoundIn::waveInErrorMsg(MMRESULT result, LPCTSTR addstr)
{
	...
	waveInGetErrorText(result, (LPWSTR)errorbuffer,100);
	ErrorMsg(_T("WAVEIN:%x:%s %s"), result, errorbuffer, addstr);
}
